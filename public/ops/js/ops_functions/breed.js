...
var BREED_FUNC = {
	        ...
	        if (selected_plumage_code == parseInt(selected_plumage_code)) {
	            ...
	            if ($('#add_plumage_menu_color').val().length > 0) {
	                ...
	                $.ajax({
	                    ...
	                    timeout: 10000,
	                    dataType: 'json',
	                    data: {data: JSON.stringify({add_plumage_menu_color: new_color, ACCESS_TOKEN: ACCESS_TOKEN})},
	                    jsonp: false,
	...
	loadForm: function(edit_id) {
	    ...
	    $.ajax({
		    ...
		    dataType: 'json',
		    cache: false,
		    data: {data: JSON.stringify({breed_id: edit_id, ACCESS_TOKEN: ACCESS_TOKEN})},
		    ...
		    error: function(a, b, c) {
		        alert('Debug warning: Could not reach server to get breed data ' + b);
		    }
	    });
	...
	addPlumageDOM: function(plumage_code, plumage_text, load_flag) {
	    ...
	    if ((APP_STATE.current_function_form_mode == 'UPDATE') && (!load_flag)) {
	        ...
	        $.ajax({
		        ...
		        dataType: 'json',
		        cache: false,
		        data: {data: JSON.stringify({breed_id: breed_id, plumage_code: plumage_code, ACCESS_TOKEN: ACCESS_TOKEN})},
		        jsonp: false,
	...
	remove_plumage: function(code) {
	    ...
	    if (APP_STATE.current_function_form_mode == 'INSERT') {
	        ...
	        $.ajax({
		        ...
		        dataType: 'json',
		        cache: false,
		        data: {data: JSON.stringify({breed_id: breed_id, plumage_code: code, ACCESS_TOKEN: ACCESS_TOKEN})},
		        jsonp: false,
	...
	getMenus: function(callback) {
		...
		$.ajax({
            ...
            error: function(a, b, c) {
                alert('Debug warning: Could not reach server to get breed menus ' + b);
            }
		});
	...
	postForm: function() {
	    ...
	    $.ajax({
	        ...
	        error: function(a, b, c) {
	            alert('Debug warning: Could not reach server to update data ' + b);
	        }
	    });
